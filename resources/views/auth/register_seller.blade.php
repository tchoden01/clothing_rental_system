...
<style>
    ...
    .auth-body {
        padding: 0.8rem 1.65rem 1.45rem;
    }
...
<section class="auth-scene">
    <div class="auth-shell">
        <header class="auth-head">
            ...
            <p>List attire and start earning</p>
        </header>

        <div class="alert auth-note">
            <i class="bi bi-info-circle"></i> Your account will be reviewed by admin before product listing is enabled.
        </div>
        ...
        <div class="auth-body">
            <form method="POST" action="{{ route('register.seller.post') }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 field-wrap">
                        <label for="shop_name" class="field-label">Shop Name</label>
                        <input type="text" class="form-control form-line @error('shop_name') is-invalid @enderror" id="shop_name" name="shop_name" value="{{ old('shop_name') }}" required>
                        @error('shop_name')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="field-wrap">
                    <label for="address" class="field-label">Shop Address</label>
                ...
                <button type="submit" class="auth-submit">Sign up as seller</button>
            </form>

            <div class="auth-links">
                Already have an account? <a href="{{ route('login') }}">Log in</a><br>
                Want to rent? <a href="{{ route('register') }}">Create customer account</a>
            </div>
        </div>
    </div>
</section>
